...
class EMDAnalysis:
    def __init__(self, **kwargs) -> None:
        ...
        # EMD分解结果误差上限
        self.error_limit = kwargs.get("error_limit", 1e-6)
    ...
    def emd(
        ...
        for i in range(max_Dectimes):
            # 提取Residue中的IMF分量
            imf = self.extractIMF(Residue, self.sifting_times, plot, **kwargs)
            if imf is None:
        ...
        if np.any(
            np.abs(np.sum(IMFs, axis=0) + Residue - datatoDec) >= self.error_limit
        ):
    ...
    def isIMF(self, imf: np.ndarray, kappa: float = 0.1, end: bool = False) -> tuple:
        ...
        if len(max_index) < 3 or len(min_index) < 3:
            if len(max_index) + len(min_index) < 2:
                return (False, imf, "极值点不足，无法提取IMF分量")
            else:
                return (True, imf, "提取出的IMF为最大周期，无法进一步Sifting")
